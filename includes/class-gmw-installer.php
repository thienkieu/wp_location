...
if ( ! class_exists( 'GMW_Installer' ) ) :
	...
	class GMW_Installer {
		...
		public static function create_tables() {
			...
			// room info table name.
			$rooms_table = $wpdb->base_prefix . 'gmw_rooms';

			// check if table already exists.
			$table_exists = $wpdb->get_results( "SHOW TABLES LIKE '{$rooms_table}'", ARRAY_A ); // WPCS: db call ok, cache ok, unprepared SQL ok.

			// if room table not exists create it.
			if ( 0 === count( $table_exists ) ) {
				// generate table sql.
				...
				object_id BIGINT(20) UNSIGNED NOT NULL default 0,
				name VARCHAR( 50 ) NOT NULL,
				number_room TINYINT NOT NULL default 0,
				avalible_room TINYINT NOT NULL default 0,				
